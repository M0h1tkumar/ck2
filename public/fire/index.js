...
// Tab Switching Logic for Agenda
function initTabs() {
    ...
    tabs.forEach(tab => {
        tab.addEventListener('click', () => {
            ...
            const activeBucket = document.getElementById(targetDay);
            if (activeBucket) {
                activeBucket.classList.add('active');
                // Scroll top of scroll-body
...
// Event Card Redirection Logic
function initEventLinks() {
    document.addEventListener('click', (e) => {
        const card = e.target.closest('.event-card');
        if (card) {
            const url = card.getAttribute('data-url');
            if (url && !url.includes('Chakravyuh2K26Registration') && url !== 'javascript:void(0)') {
                window.location.href = url;
            } else {
                alert('Registration Link will be Updated Shortly');
                console.log('Redirection link not yet set for:', card.querySelector('.event-title').textContent);
            }
        }
    });
}
